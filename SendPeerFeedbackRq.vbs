Sub Click(Source As Button)
    ...
    Forall x In peerArray
        ...
        If x.peerName <> "" And x.peerEmail <> "" And x.peerSetup <> "" And x.peerflag = x.peerEmail Then
            caseResult = "3"  'This has already been sent. Throw an error stating they already sent this and it won't be sent again.
        Elseif x.peerName <> "" And x.peerEmail <> "" And x.peerSetup <> "" Then
            caseResult = "4" 'continue on...we're good
        Elseif  x.peerName = "" And x.peerEmail <> "" And x.peerSetup <> "" Then
        ...
        Case "4":
            ...
            'body text
            Call richtext.AppendText("A Manager / Supervisor has requested that you fill out a Peer Review for " & subtext & " with the following criteria:")
